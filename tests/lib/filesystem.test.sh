...
function test_filesystem_get_command_input_dir_returns_child_dir_if_it_exists() {
  ...
  _output=$( (get_command_input_dir "$PARENT_DIR" "$CHILD_DIR_NAME") 2>&1)

  assert_equals "$_output" "$PARENT_DIR/$CHILD_DIR_NAME"
}
